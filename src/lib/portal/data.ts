...
const ROADMAP_SEED: Record<string, RoadmapItem[]> = {
  ...
  // Arco Irish. Rewritten 2026-08-20, the day Paul first signs in. The previous
  // seed was frozen at 22 July: it still read "ready to send" a month after the
  // first emails went out, and it promised an August holiday cover that never
  // shipped and whose window had already closed. A delivery log the client can
  // date-check is worth nothing when it is stale, and worse than nothing here,
  // because the one page that exists to prove we are working proved the opposite.
  "arco-irish": [
    ...
    { id: "r2", date: "2026-07-01", status: "done", kind: "milestone", title: "Retainer starts", detail: "Build & Operate plan begins.", tags: ["commercial"] },
    ...
    { id: "r8", date: "2026-07-29", status: "done", kind: "launch", title: "First campaign live", detail: "Sending began at low volume across the nine warm inboxes, Tuesday to Thursday, with a three-day follow-up. The first human reply came from the first email.", tags: ["launch"] },
    { id: "r9", date: "2026-08-04", status: "done", kind: "milestone", title: "First meeting from cold outreach", detail: "Neal Morrison, Regional Managing Partner at Dains Ireland, replied and met Paul. No live requirement at the time, but 24 hours later he introduced Paul to Karl at GSG Advisory. The first campaign produced a relationship and a referral.", tags: ["reply", "meeting"] },
    { id: "r10", date: "2026-08-17", status: "done", kind: "build", title: "Sending rebuilt on clean infrastructure", detail: "The first set of sending domains was being rejected at the receiving gateway rather than landing in spam, which is why volume was not turning into conversations. Three new domains and nine new inboxes were provisioned, delivery was tested at nine out of nine to the inbox, and the campaign was relaunched on the clean infrastructure. Zero bounces since.", tags: ["deliverability"] },
    { id: "r11", date: "2026-08-18", status: "in_progress", kind: "launch", title: "Campaign running · 405 companies", detail: "Irish SME leaders with no in-house HR function, Monday to Friday, Irish office hours, three steps three days apart. Every reply lands in the Live Deals board in this workspace.", tags: ["launch"] },
    { id: "r12", date: "2026-08-19", status: "in_progress", kind: "build", title: "Hiring-signal engine · who has an open mandate today", detail: "Eddie O'Connell at OMT Global told us he had just completed a hire and would keep Paul in mind for the next one. He had the mandate; we reached him after it closed. So we now scan every leadership role advertised in Ireland: 11,197 postings across two sources over 30 days. Stripping out recruiters, junior roles and anyone already in sequence leaves 436 Irish companies with a leadership seat open right now, and 92 of those are Arco's size and sector. Two lists are live in Target Lists, each row carrying the person to call, the role they are hiring and the date it was posted.", tags: ["signal", "leads"] },
    { id: "r13", date: "", status: "planned", kind: "build", title: "Protecting the arcoirish.com domain", detail: "Your own domain has no DMARC record, so nothing tells a receiving server to reject mail that only pretends to come from arcoirish.com. Ten minutes on a call together and it is fixed. Not billable: it is your domain and it should be protected.", tags: ["deliverability"] },
    { id: "r14", date: "", status: "planned", kind: "milestone", title: "LinkedIn alongside email", detail: "Connect with the leaders we have no email for, so the same name is reached on two channels, and turn Paul's own posts into a steady organic presence twice a month.", tags: ["linkedin", "content"] },
  ]
};
